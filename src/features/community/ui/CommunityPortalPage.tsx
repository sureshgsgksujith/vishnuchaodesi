...
function CreateGroup({done}:{done:()=>Promise<void>}){return <Form label="Create group" submit={async f=>{await communityApi.createGroup({name:f.get("name"),description:f.get("description"),visibility:f.get("visibility"),city:f.get("city"),state:f.get("state"),country:f.get("country")});await done()}}><input name="name" required minLength={3} placeholder="Group name"/><input name="country" required placeholder="Country"/><input name="state" required placeholder="State / Province"/><input name="city" required placeholder="City"/><select name="visibility" aria-label="Group visibility"><option>PUBLIC</option><option>PRIVATE</option><option>INVITE_ONLY</option></select><textarea name="description" placeholder="Description"/></Form>}
