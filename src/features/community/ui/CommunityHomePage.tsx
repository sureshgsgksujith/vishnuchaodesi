...
function CommunityLocationModal({initial,detected,close,apply}:{initial:{countryName?:string;stateName?:string;cityName?:string};detected:{countryName:string;stateName:string;cityName:string};close:()=>void;apply:(location:{countryName:string;stateName:string;cityName:string})=>void}){
  ...
  async function changeCountry(id:number){const next=countries.find(item=>item.id===id);setCountry(next);setState(undefined);setCity(undefined);setStates([]);setCities([]);if(!next)return;setLoadingStates(true);try{setStates(await getLocationStates(next.id))}finally{setLoadingStates(false)}}
  async function changeState(id:number){const next=states.find(item=>item.id===id);setState(next);setCity(undefined);setCities([]);if(!next)return;setLoadingCities(true);try{setCities(await getLocationCities(next.id))}finally{setLoadingCities(false)}}
