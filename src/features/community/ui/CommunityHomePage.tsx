...
export default function CommunityHomePage() {
  ...
  return <>
    ...
    <main className="community-home chao-home-page">
      ...
      {showLocation?<CommunityLocationModal initial={activeLocation} detected={{countryName:currentLocation.country||"",stateName:currentLocation.state||"",cityName:currentCity}} close={()=>setShowLocation(false)} apply={location=>{setHomeSelectedLocation(location);setShowLocation(false)}}/>:null}
...
function CommunityLocationModal({initial,detected,close,apply}:{initial:{countryName?:string;stateName?:string;cityName?:string};detected:{countryName:string;stateName:string;cityName:string};close:()=>void;apply:(location:{countryName:string;stateName:string;cityName:string})=>void}){
  ...
  const stateRequest=useRef(0),cityRequest=useRef(0);
  ...
  async function changeCountry(id:number){const request=++stateRequest.current;++cityRequest.current;const next=countries.find(item=>item.id===id);setCountry(next);setState(undefined);setCity(undefined);setStates([]);setCities([]);setLoadingCities(false);if(!next)return;setLoadingStates(true);try{const result=await getLocationStates(next.id);if(request===stateRequest.current)setStates(result)}finally{if(request===stateRequest.current)setLoadingStates(false)}}
  async function changeState(id:number){const request=++cityRequest.current;const next=states.find(item=>item.id===id);setState(next);setCity(undefined);setCities([]);if(!next)return;setLoadingCities(true);try{const result=await getLocationCities(next.id);if(request===cityRequest.current)setCities(result)}finally{if(request===cityRequest.current)setLoadingCities(false)}}
  return <div className="home-location-modal-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><div className="home-location-modal" role="dialog" aria-modal="true" aria-labelledby="community-location-title"><button type="button" className="home-location-modal-close" onClick={close} aria-label="Close location popup"><i className="material-icons">close</i></button><div className="home-location-modal-head"><i className="material-icons">location_on</i><div><h3 id="community-location-title">Location</h3><p>{[initial.cityName,initial.stateName,initial.countryName].filter(Boolean).join(", ")||"Choose your location"}</p></div></div><div className="home-location-select-grid"><label>Country<select value={country?.id||""} disabled={loadingCountries} autoFocus onChange={event=>void changeCountry(Number(event.target.value))}><option value="">{loadingCountries?"Loading countries":"Select Country"}</option>{countries.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>State<select value={state?.id||""} disabled={!country||loadingStates} onChange={event=>void changeState(Number(event.target.value))}><option value="">{loadingStates?"Loading states":"Select State"}</option>{states.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>City<select value={city?.id||""} disabled={!state||loadingCities} onChange={event=>setCity(cities.find(item=>item.id===Number(event.target.value)))}><option value="">{loadingCities?"Loading cities":"Select City"}</option>{cities.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select></label></div>{loadingCountries||loadingStates||loadingCities?<div className="home-location-loading"><span className="home-location-spinner"></span>{loadingCountries?"Loading countries":loadingStates?"Loading states":"Loading cities"}</div>:null}<div className="home-location-modal-actions"><button type="button" className="home-location-secondary" onClick={()=>apply(detected)} disabled={!detected.cityName}><i className="material-icons">my_location</i>Use current</button><button type="button" className="home-location-primary" disabled={!country||!state||!city} onClick={()=>country&&state&&city&&apply({countryName:country.name,stateName:state.name,cityName:city.name})}><i className="material-icons">refresh</i>Reload</button></div></div></div>}
